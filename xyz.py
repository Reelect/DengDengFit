...
warped, axis = four_point_transform(orig, screenCnt.reshape(4, 2) * ratio)
cv2.waitKey(0)
...
a4 = np.sqrt(((screenCnt[0][0] - screenCnt[1][0]) ** 2) + ((screenCnt[0][1] - screenCnt[1][1]) ** 2))*(650/warped.shape[0])
...
def DrawConnectedCircle(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if i == 0:
            ...
            if axis == 0:
                print((np.sqrt((x1 - x) ** 2 + (y1 - y) ** 2) / a4) * 29, "cm")
            else:
                print((np.sqrt((x1 - x) ** 2 + (y1 - y) ** 2) / a4) * 21, "cm")
            i = 0
...
cv2.namedWindow('123')
cv2.setMouseCallback('123', DrawConnectedCircle)
warped = imutils.resize(warped, height=650)
out = 0
i = 0
while (1):
    cv2.imshow('123', warped)
